...
# The pool hides files unless the client looks like apt. 0.6.2 is gone.
...
aisHtml=$(wget --user-agent="$AGENT" -O - "$aisDir/")
...
wget --user-agent="$AGENT" -O "$aisDeb" "$aisDir/$aisDeb"
wget --user-agent="$AGENT" -O "$webDeb" "$webDir/$webDeb"
